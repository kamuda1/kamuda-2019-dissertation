...
In this section we use learning curves to determine the number of examples in the training dataset and quantify how the online data augmentation method improved the testing error. To create the learning curves, a dataset of 3 x 10$^{8}$ spectra (1 x 10$^{4}$ examples per class) were generated for both the simple and complete dataset parameters (Tables \ref{table:hyperparameter_dataset_easy_parameters} and \ref{table:hyperparameter_dataset_full_parameters}). For each number of examples explored, a random subset of the 3 x 10$^{8}$ spectra were used as a training set. Stratified sampling was used to keep the number of examples per class as equal as possible. A holdout dataset of 300 total spectra (10 examples per class) was used as a testing dataset set. Training was ended after an early stopping condition with a patience of 100 epochs was reached. A total of 5 models were trained for the DNN, CNN, DAE, and CAE and their outputs were averaged to reduce variance between models. Online data augmentation was also implemented using an early stopping patience of 50 and 100 for the simple and complete models.

The learning curve for the simple model shows that past 5000 examples the F1 score saturates to one. This shows that each model is well suited to generalize within the simple parameter space. Due to the error not decreasing during training, training was rerun on one CAE model at 1 x 10$^{4}$ and 2 x 10$^{4}$ samples. This can be attributed to a poor CAE pretraining. This poor pretraining can also be seen in the CAE's poor online data augmentation performance. The performance of each network trained with online data augmentation was poor.
...
The training set error curves in the complete learning curve plot are less steep than those in the simple learning curve plot. This shows that the complete dataset is more complex and models require more examples to generalize to unseen examples. The learning curve for the complete model shows the CNN and DNN converging to near one after 5 x 10$^{3}$ and 1.5 x 10$^{4}$ examples. The testing and training F1 score curves for the CNN and DNN also show low-bias and low-variance, indicating the hyperparameter search found appropriate values for the complexity of the complete dataset. 
