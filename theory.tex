...
to reduce the error metric for some data. The learning rate and its affect on training are discussed further in section \ref{LearningRateSubsection}. Through reducing the error, the network learns a task for some data.

Backpropagation uses the chain rule to find these derivatives for some training data and target, 
%
...
\end{align}
%
Due to computational constraints, it is often infeasible to calculate the update rules in Equations \ref{eq:update1} and \ref{eq:update2} for the entire training dataset.
