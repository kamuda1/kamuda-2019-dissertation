...
%
\begin{align} \label{eq:template_sampling}
C_{total}(E) &= t b_{cps} B(E) + t s_{cps} \sum_{n=1}^{N} c_{n} C_{n}(E)
\intertext{where}
C_{total}(E) &= \text{Counts in energy bin E for the simulated spectrum} \nonumber \\
B(E) &= \text{Counts in energy bin E for the background template} \nonumber \\
C_{n}(E) &= \text{Counts in energy bin E for n$^{th}$ source template} \nonumber \\
