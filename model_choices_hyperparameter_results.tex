...
To investigate which parameters affect the convolutional and dense model's performance on simple and more complicated datasets, two separate hyperparameter searches are employed for each model. The hyperparameter search is illustrated in Figure \ref{fig:hyperparameter_search_workflow}. For each dataset, 256 sets of random hyperparameters are chosen and 

\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{images/hyperparameter_search_workflow}
...
\subsection{Datasets Used for the Hyperparameter Search}
